...
class AvailabilityScreenTest2 {
    @get:Rule
    val composeTestRule = createComposeRule()

    private class FakeListProfilesViewModel : ListProfilesViewModel(mock(ProfilesRepository::class.java)) {
    ...
    fun noProfileSelectedTest() {
        ...
        composeTestRule.setContent {
            AvailabilityScreen(navigationActions, viewModel)
        }

        // When the composable is set
        // Then
        composeTestRule.onNodeWithText("No Profile selected. Should not happen.", useUnmergedTree = true)
            .assertIsDisplayed()
    }
}
