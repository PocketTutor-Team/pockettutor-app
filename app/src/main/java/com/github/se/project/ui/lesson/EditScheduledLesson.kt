...
fun EditScheduledLessonScreen(
  ...
  var timeSlot by remember {
    mutableStateOf(Calendar.getInstance().apply { time = Date(lesson.timeSlot) })
  }
  ...
  val datePickerDialog =
      DatePickerDialog(
          ...
          timeSlot.get(Calendar.YEAR),
          timeSlot.get(Calendar.MONTH),
          timeSlot.get(Calendar.DAY_OF_MONTH))
  ...
  val timePickerDialog =
      TimePickerDialog(
          ...
          timeSlot.get(Calendar.HOUR_OF_DAY),
          timeSlot.get(Calendar.MINUTE),
          true)
  ...
  Column(modifier = Modifier.padding(16.dp).fillMaxWidth()) {
    ...
    Row(modifier = Modifier.fillMaxWidth()) {
      Button(onClick = { datePickerDialog.show() }) { Text("Select Date") }
      Spacer(modifier = Modifier.width(8.dp))
      Button(onClick = { timePickerDialog.show() }) { Text("Select Time") }
    }
    TextField(
        value = price,
    ...
    Button(
        onClick = { lessonViewModel.deleteLesson(lesson.id) { onCancel() } },
        modifier = Modifier.padding(vertical = 8.dp)) {
          Text("Cancel Lesson")
        }
  }
