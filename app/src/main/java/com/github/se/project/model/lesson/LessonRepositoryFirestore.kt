...
class LessonRepositoryFirestore(private val db: FirebaseFirestore) : LessonRepository {
    ...
    override fun getLessonsByUserId(
        ...
        db.collection("users").document(userUid).collection(collectionPath).get()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    val lessons = task.result?.mapNotNull { document -> documentToLesson(document) } ?: emptyList()
                    onSuccess(lessons)
                } else {
    ...
    override fun addLessonByUserId(
        ...
        db.collection("users").document(userUid).collection(collectionPath)
            .document(lesson.id).set(lesson)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    onSuccess()
                } else {
