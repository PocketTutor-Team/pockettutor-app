...
class LessonRepositoryFirestore(private val db: FirebaseFirestore) : LessonRepository {
            ...
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    task.result?.let { querySnapshot ->
                        ...
                        // Log the size of the lessons list after conversion
                        Log.d("lessons filled", "Fetched lessons successfully, size: ${lessons.size}")
    ...
    private fun documentToLesson(document: DocumentSnapshot): Lesson? {
        return try {
            ...
            val lesson = Lesson(id, title, description, tutorUid, studentUid, price, timeSlot, status, language)
