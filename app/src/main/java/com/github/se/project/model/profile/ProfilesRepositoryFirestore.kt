...
class ProfilesRepositoryFirestore(private val db: FirebaseFirestore) : ProfilesRepository {
    ...
    override fun addProfile(profile: Profile, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        ...
        val task: Task<Void> = db.collection(collectionPath).document(uploadProfile.uid).set(uploadProfile)
    ...
    override fun updateProfile(
        ...
        val uploadProfile = profileUpload(profile)
        performFirestoreOperation(db.collection(collectionPath).document(uploadProfile.uid).set(uploadProfile), onSuccess, onFailure)
    }
    ...
    private fun documentToProfile(document: DocumentSnapshot): Profile? {
        return try {
            ...
            //Note that in the above section, we could do "Section.valueOf([current expression]).NAME", to check the string corresponds to the enums, at the cost of making the code less legible
            val uploadProfile = ProfileUpload(uid, firstName, lastName, phoneNumber, role, section, academicLevel)
