...
exports.pushNotificationTutorLessonConfirmation = onDocumentUpdated(
  ...
  async (event) => {
    ...
    if (before.status !== after.status) {
      console.log(`Lesson ${lessonId} status changed from ${before.status} to ${after.status}`);
      ...
      if (before.status === "PENDING_TUTOR_CONFIRMATION" && after.status === "CONFIRMED") {
        try {
          ...
          const tutorDoc = await admin.firestore().collection("profiles").doc(tutorUid).get();
          const studentDoc = await admin.firestore().collection("profiles").doc(studentUid).get();
          ...
          if (!tutorToken || !studentToken || !tutorName || !studentName) {
            console.error("Missing required fields in profiles. Notification not sent.");
            return;
          }
          ...
          const payloadStudent = {
            notification: {
              ...
              body: `Your lesson with ${tutorName} has been confirmed. See you on ${after.timeSlot}!`,
            },
            token: studentToken,
          };
          ...
          const payloadTutor = {
            notification: {
              title: "Lesson Confirmed!",
              body: `Your lesson with ${studentName} has been confirmed. See you on ${after.timeSlot}!`,
            },
            token: tutorToken,
          };
          ...
          console.error(`Error sending notifications for lesson ${lessonId}:`, error);
        }
      } else {
